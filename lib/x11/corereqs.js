...
module.exports = {
   CreateWindow: [
       ...
       function(id, parentId, x, y, width, height, borderWidth, _class, visual, values) {
           ...
           // xproto docs "A depth of zero for class InputOutput or CopyFromParent means the depth is taken from the parent."
           var depth = 0;
	   ...
	   var res = buf.unpack('SSSxL');
		...
		function(colormap, name) {
			var padded = xutil.padded_string(name);
			 // params: colormap, name(STRING8)
			 return ['CxSLLa', [85, 3+padded.length/4, colormap, name.length, padded] ];
			},
			function(buf) {
				var res = buf.unpack('LSSSSSS');
				var color = {
					pixel:  res[0],
					red:	res[1],
					green:  res[2],
