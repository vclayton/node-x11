...
UnpackStream.prototype.getAppend = function(length, appendTo, callback)
{
	if (0 == length) {
		callback(appendTo);
		return;
	}
	var fixed_request = new ReadFixedRequest(length + appendTo.length, callback);
	appendTo.copy(fixed_request.data);
	fixed_request.received_bytes = appendTo.length;
	this.read_queue.push(fixed_request);
	this.resume();
}
